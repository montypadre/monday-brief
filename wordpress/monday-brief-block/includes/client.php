...
function mbb_fetch_summary(): array
{
    ...
    if (is_array($cached)) {
        return array('ok' => true, 'data' => $cached, 'error' => null, 'stale' => false);
    }
    ...
    return array('ok' => true, 'data' => $data, 'error' => null, 'stale' => false);
}
...
function mbb_format_delta(array $card): array
{
    if (! isset($card['changePct']) || null === $card['changePct']) {
        return array('text' => __('no earlier period to compare', 'monday-brief-block'), 'direction' => 'flat');
    }
